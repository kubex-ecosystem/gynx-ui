...
import { Box, User, Lock, Mail, CheckCircle2, AlertCircle, Loader2, Eye, EyeOff, ArrowRight, ShieldCheck, Building } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import Card from '../components/ui/Card';
import { InviteDTO, AcceptInviteReq } from '../types';
  ...
  if (isValidating) {
    return (
      <div className="min-h-screen bg-main flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-muted uppercase tracking-widest text-xs font-bold">Validando seu convite...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-main flex items-center justify-center px-4">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full">
          <Card className="p-8 text-center space-y-6 border-status-success/30 bg-status-success/5">
            ...
            <div className="space-y-2">
              <h2 className="text-3xl font-bold text-primary">Bem-vindo à GNyx!</h2>
              <p className="text-secondary text-sm">Seu cadastro foi concluído com sucesso no ecossistema Bellube.</p>
            </div>
            <div className="p-4 rounded-2xl bg-surface-primary/50 border border-border-primary text-left flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-primary">E-mail de confirmação enviado</p>
                <p className="text-[10px] text-muted">Enviamos os detalhes do seu acesso para <strong>{invite?.email}</strong>.</p>
              </div>
            </div>
            <button 
    ...
    <div className="min-h-screen bg-main text-primary flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-lg space-y-8 animate-fade-in relative z-10">
        <div className="text-center space-y-4">
          ...
          <h1 className="text-4xl font-bold tracking-tight">Finalizar Cadastro</h1>
          <p className="text-secondary text-sm">Você foi convidado para participar da organização GNyx.</p>
        </div>
        ...
        <Card className="bg-surface-primary/80 backdrop-blur-xl border-border-primary/50 shadow-2xl p-8 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl bg-accent-muted/20 border border-accent-primary/20">
            ...
            <div className="text-center sm:text-left">
              <p className="text-[10px] font-bold text-accent-secondary uppercase tracking-[0.2em]">Organização</p>
              <p className="text-sm font-bold text-primary">{invite?.tenant_id.replace('tenant_', '').toUpperCase()}</p>
              <p className="text-xs text-muted">Acesso como: <span className="text-accent-secondary font-mono">{invite?.role}</span></p>
            </div>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted uppercase tracking-[0.2em]">Nome</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted uppercase tracking-[0.2em]">Sobrenome</label>
                <div className="relative">
                  <User className="absolute left-3 top-3 text-secondary" size={18} />
                  <input
                    type="text"
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted uppercase tracking-[0.2em]">E-mail (Identificado)</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-muted uppercase tracking-[0.2em]">Defina sua Senha</label>
              <div className="relative">
                ...
                >
                  {showPassword ? <EyeOff size={18} className="opacity-80" /> : <Eye size={18} className="opacity-80" />}
                </button>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-surface-secondary/30 border border-border-primary flex items-start gap-3">
              ...
              <p className="text-[10px] text-secondary leading-relaxed italic">
                Ao clicar em finalizar, você concorda com os termos de uso e política de privacidade da Kubex Cloud para a infraestrutura GNyx.
              </p>
            </div>
