...
export default function WorkspaceSettings() {
  ...
  const {
    ...
    restoreDefaults,
    retry,
  } = useWorkspaceSettings(workspaceScope);
    ...
    <div className="space-y-8 animate-fade-in relative">
      <header className="flex flex-col gap-2 relative z-10">
        ...
        <p className="text-secondary text-sm">
          Ajustes administrativos, operação do Tenant e simulação do ambiente
          visual.
        </p>
      </header>
        ...
        <Card className="p-4 border-status-error/30 bg-status-error/5">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-primary">
                  Falha ao preparar a tela de workspace
                </p>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <div className="p-8 lg:p-12 space-y-6 relative z-10">
            ...
            <p className="text-sm text-secondary leading-relaxed">
              Este painel controla as diretivas da organização atual que foi
              processada pelo Gateway. Use os controladores abaixo para moldar a
              interface para a sua demonstração comercial sem ferir o Back-End
              Go.
            </p>

            {activeTenant && (
              <div className="rounded-2xl border border-border-primary/70 bg-surface-primary/50 px-4 py-3 text-xs text-secondary">
            ...
            {isSimulated && (
              <div className="p-3 border-l-2 border-status-warning bg-status-warning/10 text-status-warning text-xs font-mono">
                [DEV MODE]: As alterações aqui refletirão visualmente na UI da
                sessão. O tenant ativo do auth context continua sendo a fonte de
                verdade para o escopo atual.
              </div>
            )}
          </div>
            ...
            <div className="rounded-2xl border border-border-primary bg-surface-secondary/40 p-6 flex items-center gap-3 text-secondary">
              ...
              <div>
                ...
                <p className="text-xs text-muted">
                  Montando estado inicial do workspace.
                </p>
              </div>
            </div>
          ) : (
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xl font-bold text-primary mt-1">
                  {formData?.billingPlan ?? "Indisponivel"}
                </p>
                <p className="text-xs text-secondary mt-1">
                  Recursos ilimitados habilitados no ambiente local.
                </p>
              </div>
          ...
          <Card className="p-6 border-border-secondary bg-surface-primary/40 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-secondary mt-2 mb-4 leading-relaxed">
                Gerencie os R.B.A.C, convites e permissões de acesso ao
                workspace para o time de engeharia.
              </p>

              <button
                onClick={() =>
                  navigateToSection("accept-invite", {
                    token: "demo-workspace-invite",
                  })
                }
