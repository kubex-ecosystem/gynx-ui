...
export const useWorkspaceSettings = () => {
  ...
  const [savedSnapshot, setSavedSnapshot] = useState<WorkspaceSettingsData | null>(null);
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      setError(resolveErrorMessage(loadError, 'Nao foi possivel carregar as configuracoes do workspace.'));
      setFormData(null);
  ...
  const updateField = useCallback(
    <K extends keyof WorkspaceSettingsData>(field: K, value: WorkspaceSettingsData[K]) => {
  ...
  const save = useCallback(async () => {
    ...
    try {
      const persisted = await workspaceService.saveSettings(formData);
      ...
      const message = resolveErrorMessage(saveError, 'Nao foi possivel salvar as configuracoes do workspace.');
      setError(message);
      setStatus({
        tone: 'error',
        message,
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    try {
      const defaults = await workspaceService.resetSettings();
      ...
      const message = resolveErrorMessage(resetError, 'Nao foi possivel restaurar os defaults do workspace.');
      setError(message);
      setStatus({
        tone: 'error',
        message,
