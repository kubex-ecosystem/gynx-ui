      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p
              className={`text-sm font-semibold truncate ${isDirectActive ? "text-inherit" : "text-primary"}`}
            >
              {section.label}
            </p>
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div
        className={`hidden px-6 pt-8 pb-5 ${collapsed ? "lg:hidden" : "lg:block"}`}
      >
        ...
        <p className="mt-2 text-sm text-secondary">{t("sidebarDescription")}</p>
      </div>

      <div
        className={`hidden items-center justify-center gap-3 px-3 pt-6 pb-4 ${collapsed ? "lg:flex" : "lg:hidden"}`}
      >
      ...
      <nav
        className={`flex-1 overflow-y-auto custom-scrollbar px-4 pb-6 ${collapsed ? "lg:hidden" : "lg:px-6"}`}
      >
      ...
      {/* Mini Sidebar Nav */}
      <nav
        className={`hidden flex-1 overflow-y-auto custom-scrollbar pb-6 ${collapsed ? "lg:flex" : "lg:hidden"}`}
      >
        <ul className="flex w-full flex-col items-center gap-3">
          {sections.map((section) => {
            const Icon =
              section.icon || defaultIcons[section.id] || LayoutDashboard;
            ...
            return (
              <li key={section.id} className="w-full">
                <button
                  ...
                  className={`mx-auto flex h-12 w-12 items-center justify-center rounded-2xl border text-secondary transition-all duration-200 ${
                    isActive
      ...
      <div
        className={`hidden border-t border-border-primary px-6 py-5 text-xs text-secondary ${collapsed ? "lg:hidden" : "lg:block"}`}
      >
