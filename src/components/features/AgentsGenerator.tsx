...
const i18n: Record<string, Record<string, string>> = {
  en: {
    ...
    overviewDescription:
      "Configure squads, generate AGENTS.md and govern your AI estate.",
    ...
    workspaceDescription:
      "Capture requirements and configure frameworks before generating squads.",
    contextCardTitle: "Context & Ideas",
    ideaInputPlaceholder: "Document a requirement, hypothesis or constraint...",
    addIdea: "Add idea",
    ideasEmpty:
      "Start with raw requirements – they will be translated into agents.",
    ...
    noGeneratedAgents:
      "Generate a squad to preview the Markdown and export AGENTS.md.",
    ...
    storedAgentsDescription:
      "Agents persisted on the server (internal JSON store).",
    ...
    providerReady: "Ready",
    generationFailed:
      "Generation failed. Please confirm the unified backend is reachable.",
    requirementsMissing:
      "Add at least one idea and define provider + role before generating.",
  ...
  pt: {
    ...
    overviewDescription:
      "Configure squads, gere o AGENTS.md e governe seu ecossistema de IA.",
    frameworksLabel: "Framework",
    providersLabel: "Provider",
    ...
    workspaceDescription:
      "Capture requisitos e ajuste frameworks antes de gerar squads.",
    contextCardTitle: "Contexto & Ideias",
    ideaInputPlaceholder: "Anote um requisito, hipótese ou constraint...",
    addIdea: "Adicionar ideia",
    ideasEmpty:
      "Comece registrando requisitos brutos — eles serão convertidos em agents.",
    ...
    noGeneratedAgents:
      "Gere um squad para visualizar o Markdown e exportar o AGENTS.md.",
    ...
    storedAgentsDescription:
      "Agents persistidos no servidor (store interno em JSON).",
    ...
    generationFailed:
      "Falha na geração. Confirme se o backend unificado está acessível.",
    requirementsMissing:
      "Adicione ao menos uma ideia e defina provider + papel antes de gerar.",
  ...
  es: {
    ...
    overviewDescription:
      "Configura squads, genera AGENTS.md y gobierna tu portafolio de IA.",
    frameworksLabel: "Framework",
    ...
    workspaceDescription:
      "Captura requisitos y ajusta frameworks antes de generar squads.",
    contextCardTitle: "Contexto e Ideas",
    ideaInputPlaceholder:
      "Registra un requerimiento, hipótesis o restricción...",
    ...
    noGeneratedAgents:
      "Genera un squad para ver el Markdown y exportar AGENTS.md.",
    copyMarkdown: "Copiar Markdown",
    copyRequirements: "Copiar requisitos",
    ...
    storedAgentsTitle: "Agents Registrados",
    storedAgentsDescription:
      "Agents persistidos en el servidor (store interno JSON).",
    ...
    providerReady: "Listo",
    generationFailed:
      "Falló la generación. Confirma que el backend unificado está disponible.",
    requirementsMissing:
      "Agrega al menos una idea y define provider + rol antes de generar.",
...
const AgentsGenerator: React.FC<AgentsGeneratorProps> = (
  ...
  const handlePersistBlueprint = async (
    entry: BlueprintEntry | null = currentBlueprint,
  ) => {
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl border border-border-primary bg-surface-primary/60 p-4 text-sm text-muted">
            ...
            <p className="mt-2 text-lg font-semibold text-primary">
              {frameworks.find((fw) => fw.value === agentFramework)?.label}
            </p>
          </div>
          <div className="rounded-xl border border-border-primary bg-surface-primary/60 p-4 text-sm text-muted">
            <p className="text-xs uppercase tracking-[0.3em] text-secondary">
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className="text-lg font-semibold text-primary">
                {(selectedProviderInfo?.display_name ?? agentProvider) || "--"}
              </span>
              {selectedProviderInfo && (
                <span
                  className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${providerStatusTone}`}
                >
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-secondary dark:text-muted">
                      {entry.result.agents.length}{" "}
                      {t("generatedAgents").toLowerCase()}
                    </p>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#111827]/70 backdrop-blur-sm px-4 py-8">
          <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-3xl border /95 shadow-2xl border-border-primary bg-surface-primary/95">
            <header className="sticky top-0 flex items-center justify-between border-b /95 px-6 py-4 border-border-primary bg-surface-primary/95">
              <div>
                <h2 className="text-lg font-semibold text-[#0f172a] text-primary">
                  {t("generationTitle")}
                </h2>
                <p className="text-xs text-secondary dark:text-muted">
                  {t("generatedAt")}:{" "}
                  {formatTimestamp(currentBlueprint.createdAt)}
                </p>
              </div>
              <button
                type="button"
              ...
              <section>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={isSaving ||
                        !currentBlueprint.result.agents.length}
                ...
                <div className="mt-3 space-y-3">
                  {currentBlueprint.result.agents.map((agent, index) => (
                    ...
                    >
                      <p className="text-sm font-semibold text-[#0f172a] text-primary">
                        {agent.Title}
                      </p>
