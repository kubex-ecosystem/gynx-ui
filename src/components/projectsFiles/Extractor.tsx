...
export default function ProjectExtractor({ projectFile, projectName, description }: ProjectExtractorProps) {
 ...
 <div className="text-2xl font-semibold text-accent-primary">{projectData.stats.totalFiles}</div>
 <div className="text-sm text-secondary dark:text-muted">{t('files')}</div>
 </div>
 <div className="rounded-xl border border-[#e2e8f0] bg-surface-secondary/80 p-3 shadow-sm border-border-primary bg-surface-primary/80">
 <div className="text-2xl font-semibold text-[#0891b2]">{formatFileSize(projectData.stats.totalBytes)}</div>
 <div className="text-sm text-secondary dark:text-muted">{t('totalSizeLabel')}</div>
 </div>
 <div className="rounded-xl border border-[#e2e8f0] bg-surface-secondary/80 p-3 shadow-sm border-border-primary bg-surface-primary/80">
 <div className="text-2xl font-semibold text-[#a855f7]">{projectData.stats.totalMarkers}</div>
 <div className="text-sm text-secondary dark:text-muted">{t('fragmentsLabel')}</div>
 </div>
 </div>
 ...
 isDragging && 'bg-[#ecfeff] dark:bg-[#0a1e2b] border-4 border-dashed border-accent-primary'
 )}
 ...
 <div className="mt-6 p-4 border-2 border-dashed border-[#cbd5e1] dark:border-[#334155] rounded-xl max-w-md mx-auto hover:border-accent-primary dark:hover:border-accent-primary transition-colors cursor-pointer"
 onClick={triggerFileUpload}
 >
...
function EmptyState() {
 ...
 <h3 className="text-lg font-medium mb-2 text-secondary text-secondary">{t('noFileSelected')}</h3>
...
function TreeView({
 ...
 {getFileIcon(n.path)}
 <span className={classNames('font-medium truncate', sel ? 'text-[#0f172a] text-primary' : 'text-secondary dark:text-muted')}>
 {n.name}
 </span>
 </div>
 <div className="text-xs text-muted dark:text-secondary truncate">{n.path}</div>
 </button>
 </li>
 );
 }
 ...
 >
 <FolderIcon className="w-4 h-4 text-accent-primary" />
 <span className="font-semibold text-primary text-primary">{n.name}</span>
 </button>
